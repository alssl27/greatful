    ...
    <div className="min-h-screen stripe-pattern p-4 md:p-8 relative text-white">
      <div className="max-w-6xl mx-auto text-white relative z-10">
        <motion.button
          ...
          className="flex items-center gap-2 text-white bg-black/80 px-4 py-2 rounded-full font-bold uppercase tracking-tighter mb-8 hover:bg-neon-cyan hover:text-black transition-colors border border-white/20 shadow-[0_0_18px_rgba(255,43,214,0.25)] pulse-neon"
        >
        ...
        <motion.div
          ...
          className="surface-dark neon-radiance p-8 rounded-3xl border-4 border-neon-pink"
        >
          <div className="flex items-center justify-between mb-6 border-b-2 border-white/15 pb-2">
            <h1 className="text-3xl md:text-4xl font-display italic text-white flex items-center gap-2 drop-shadow-[0_0_14px_rgba(0,245,255,0.35)]">
              <BookOpen className="w-6 h-6" />
              Saved Entries
            </h1>
            <span className="bg-neon-pink text-black text-xs px-2 py-1 rounded font-bold shadow-[0_0_12px_rgba(255,43,214,0.55)] pulse-neon">{history.length}</span>
          </div>
            ...
            <AnimatePresence mode="popLayout">
              {history.length === 0 ? (
                <motion.div
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  className="text-center py-16 opacity-50 italic"
                >
                  <Sparkles className="w-10 h-10 mx-auto mb-3 text-neon-pink drop-shadow-[0_0_16px_rgba(255,43,214,0.75)] pulse-neon" />
                ...
                history.map((entry, index) => (
                  <motion.div
                    ...
                    className="surface-light p-4 rounded-xl border-l-4 border-neon-pink text-black"
                  >
                    <p className="text-[10px] font-bold text-black/55 uppercase">{entry.date}</p>
                    <p className="text-sm font-bold text-black italic mb-1">Q: {entry.prompt}</p>
                    <p className="text-black/75 leading-relaxed font-medium whitespace-pre-line">{entry.answer}</p>
                  </motion.div>
                ))
              )}
            </AnimatePresence>
